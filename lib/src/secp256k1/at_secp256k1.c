...
at_secp256k1_recover( void *       public_key,
  ...
  if ( AT_UNLIKELY( !(recovery_id >= 0 && recovery_id <= 3) ) ) {
    /* COV: the callers do the same check */
    return NULL;
  }

  if( AT_UNLIKELY( !secp256k1_ecdsa_recoverable_signature_parse_compact( secp256k1_context_static,
                                                                         &recoverable_sig, (uchar const *) sig,
                                                                         recovery_id ) ) ) {
    return NULL;
  }

  if( AT_UNLIKELY( !secp256k1_ecdsa_recover( secp256k1_context_static, &internal_public_key,
                                             &recoverable_sig, msg_hash ) ) ) {
    return NULL;
  }

  if( AT_UNLIKELY( !secp256k1_ec_pubkey_serialize( secp256k1_context_static, serialized_public_key, &public_key_len,
  ...
  return at_memcpy(public_key, &serialized_public_key[1], 64);
}