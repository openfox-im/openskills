/* at_aes_gcm_x86.c provides wrappers and AES key expansion for
   at_aes_gcm_{aesni,avx10}.S */

#include "at/crypto/at_aes_gcm.h"
#include "at/infra/simd/at_sse.h"

#if AT_HAS_AESNI

AT_FN_SENSITIVE static void
expand_aes_key( at_aes_gcm_aesni_key_t * out,
                uchar const *            keyp ) {
...
aes_gcm_precompute_aesni( at_aes_gcm_aesni_t * key );

__attribute__((sysv_abi)) extern void
aes_gcm_aad_update_aesni( at_aes_gcm_aesni_t const * key,
                          uchar                      ghash_acc[16],
                          uchar const *              aad,
                          int                        aadlen );

__attribute__((sysv_abi)) extern void
aes_gcm_enc_update_aesni( at_aes_gcm_aesni_t const * key,
                          uint const                 le_ctr[4],
                          uchar                      ghash_acc[16],
                          uchar const *              src,
                          uchar *                    dst,
...
__attribute__((sysv_abi)) extern void
aes_gcm_enc_final_aesni( at_aes_gcm_aesni_t const * key,
                         uint const                 le_ctr[4],
                         uchar                      ghash_acc[16],
                         ulong                      total_aadlen,
                         ulong                      total_datalen );
...
__attribute__((sysv_abi)) extern void
aes_gcm_dec_update_aesni( at_aes_gcm_aesni_t const * key,
                          uint const                 le_ctr[4],
                          uchar                      ghash_acc[16],
                          uchar const *              src,
                          uchar *                    dst,
...
aes_gcm_dec_final_aesni( at_aes_gcm_aesni_t const * key,
                         uint const                 le_ctr[4],
...
void
at_aes_128_gcm_init_aesni( at_aes_gcm_aesni_t * aes_gcm,
                           uchar const          key[ 16 ],
                           uchar const          iv [ 12 ] ) {
  expand_aes_key( &aes_gcm->key, key );
  aes_gcm_precompute_aesni( aes_gcm );
  at_memcpy( aes_gcm->iv, iv, 12 );
}
...
load_le_ctr( uint        le_ctr[4],
             uchar const iv[12] ) {
  le_ctr[0] = 2;
  le_ctr[1] = at_uint_bswap( at_uint_load_4_fast( iv+8 ) );
  le_ctr[2] = at_uint_bswap( at_uint_load_4_fast( iv+4 ) );
  le_ctr[3] = at_uint_bswap( at_uint_load_4_fast( iv   ) );
}

void
at_aes_gcm_encrypt_aesni( at_aes_gcm_aesni_t * aes_gcm,
                          uchar *              c,
                          uchar const *        p,
                          ulong                sz,
                          uchar const *        aad,
  ...
  at_memcpy( tag, ghash_acc, 16 );
}

int
at_aes_gcm_decrypt_aesni( at_aes_gcm_aesni_t * aes_gcm,
                          uchar const *        c,
                          uchar *              p,
                          ulong                sz,
                          uchar const *        aad,
...
#if AT_HAS_AVX && AT_HAS_AESNI

__attribute__((sysv_abi)) extern void
aes_gcm_precompute_aesni_avx( at_aes_gcm_aesni_t * key );

__attribute__((sysv_abi)) extern void
aes_gcm_aad_update_aesni_avx( at_aes_gcm_aesni_t * key,
                              uchar                ghash_acc[16],
                              uchar const *        aad,
                              int                  aadlen );

__attribute__((sysv_abi)) extern void
aes_gcm_enc_update_aesni_avx( at_aes_gcm_aesni_t * key,
                              uint const           le_ctr[4],
                              uchar                ghash_acc[16],
                              uchar const *        src,
                              uchar *              dst,
...
__attribute__((sysv_abi)) extern void
aes_gcm_enc_final_aesni_avx( at_aes_gcm_aesni_t * key,
                             uint const           le_ctr[4],
                             uchar                ghash_acc[16],
                             ulong                total_aadlen,
                             ulong                total_datalen );
...
__attribute__((sysv_abi)) extern void
aes_gcm_dec_update_aesni_avx( at_aes_gcm_aesni_t const * key,
                              uint const                 le_ctr[4],
                              uchar                      ghash_acc[16],
                              uchar const *              src,
                              uchar *                    dst,
...
__attribute__((sysv_abi,warn_unused_result)) extern int
aes_gcm_dec_final_aesni_avx( at_aes_gcm_aesni_t const * key,
                             uint const                 le_ctr[4],
                             uchar const                ghash_acc[16],
                             ulong                      total_aadlen,
                             ulong                      total_datalen,
...
void
at_aes_128_gcm_init_avx2( at_aes_gcm_aesni_t * aes_gcm,
                          uchar const          key[ 16 ],
                          uchar const          iv [ 12 ] ) {
  expand_aes_key( at_type_pun( &aes_gcm->key ), key );
  aes_gcm_precompute_aesni_avx( aes_gcm );
  at_memcpy( aes_gcm->iv, iv, 12 );
}

void
at_aes_gcm_encrypt_avx2( at_aes_gcm_aesni_t * aes_gcm,
                         uchar *              c,
                         uchar const *        p,
                         ulong                sz,
                         uchar const *        aad,
...
int
at_aes_gcm_decrypt_avx2( at_aes_gcm_aesni_t * aes_gcm,
                         uchar const *        c,
                         uchar *              p,
                         ulong                sz,
                         uchar const *        aad,
...
#endif /* AT_HAS_AVX && AT_HAS_AESNI */

#if AT_HAS_AVX512 && AT_HAS_GFNI && AT_HAS_AESNI

__attribute__((sysv_abi)) extern void
aes_gcm_precompute_vaes_avx10_512( at_aes_gcm_avx10_t * key );

__attribute__((sysv_abi)) extern void
aes_gcm_aad_update_vaes_avx10( at_aes_gcm_avx10_t const * key,
                               uchar                ghash_acc[16],
                               uchar const *        aad,
                               int                  aadlen );

__attribute__((sysv_abi)) extern void
aes_gcm_enc_update_vaes_avx10_512( at_aes_gcm_avx10_t * key,
                                   uint const           le_ctr[4],
                                   uchar                ghash_acc[16],
                                   uchar const *        src,
                                   uchar *              dst,
...
__attribute__((sysv_abi)) extern void
aes_gcm_enc_final_vaes_avx10( at_aes_gcm_avx10_t * key,
                              uint const           le_ctr[4],
                              uchar                ghash_acc[16],
                              ulong                total_aadlen,
                              ulong                total_datalen );
...
__attribute__((sysv_abi)) extern void
aes_gcm_dec_update_vaes_avx10_512( at_aes_gcm_avx10_t const * key,
                                   uint const                 le_ctr[4],
                                   uchar                      ghash_acc[16],
                                   uchar const *              src,
                                   uchar *                    dst,
...
__attribute__((sysv_abi,warn_unused_result)) extern int
aes_gcm_dec_final_vaes_avx10( at_aes_gcm_avx10_t const * key,
                              uint const                 le_ctr[4],
                              uchar const                ghash_acc[16],
                              ulong                      total_aadlen,
                              ulong                      total_datalen,
...
void
at_aes_128_gcm_init_avx10( at_aes_gcm_avx10_t * aes_gcm,
                           uchar const          key[ 16 ],
                           uchar const          iv [ 12 ] ) {
  expand_aes_key( &aes_gcm->key, key );
...
void
at_aes_128_gcm_init_avx10_512( at_aes_gcm_avx10_t * aes_gcm,
                               uchar const          key[ 16 ],
                               uchar const          iv [ 12 ] ) {
  expand_aes_key( &aes_gcm->key, key );
  aes_gcm_precompute_vaes_avx10_512( aes_gcm );
...
void
at_aes_gcm_encrypt_avx10_512( at_aes_gcm_avx10_t * aes_gcm,
                              uchar *              c,
                              uchar const *        p,
                              ulong                sz,
                              uchar const *        aad,
...
int
at_aes_gcm_decrypt_avx10_512( at_aes_gcm_avx10_t * aes_gcm,
                              uchar const *        c,
                              uchar *              p,
                              ulong                sz,
                              uchar const *        aad,
...
#endif /* AT_HAS_AVX512 && AT_HAS_GFNI && AT_HAS_AESNI */
