...
AT_STATIC_ASSERT( AT_SHA512_BATCH_MAX==4UL, compat );
...
at_sha512_private_batch_avx( ulong          batch_cnt,
  ...
  ulong batch_tail_data[ AT_SHA512_BATCH_MAX ] __attribute__((aligned(32)));
  ulong batch_tail_rem [ AT_SHA512_BATCH_MAX ] __attribute__((aligned(32)));
...
#     endif
  ...
  wv_t wv_128     = wv_bcast( AT_SHA512_PRIVATE_BUF_MAX );
  ...
  wv_t block_rem  = wv_notczero( batch_lane, wv_add( wv_shr( wv_ld( batch_sz ), AT_SHA512_PRIVATE_LG_BUF_MAX ), tail_rem ) );
  for(;;) {
    wc_t active_lane = wv_to_wc( block_rem );
    if( AT_UNLIKELY( !wc_any( active_lane ) ) ) break;
