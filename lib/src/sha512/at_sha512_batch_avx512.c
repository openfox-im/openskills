...
AT_STATIC_ASSERT( AT_SHA512_BATCH_MAX==8UL, compat );

void
at_sha512_private_batch_avx( ulong          batch_cnt,
                             void const *   batch_data,
                             ulong const *  batch_sz,
                             void * const * batch_hash );

void
at_sha512_private_batch_avx512( ulong          batch_cnt,
                                void const *   _batch_data,
                                ulong const *  batch_sz,
                                void * const * _batch_hash ) {

  if( AT_UNLIKELY( batch_cnt<5UL ) ) {
    at_sha512_private_batch_avx( batch_cnt, _batch_data, batch_sz, _batch_hash );
    return;
  }
  ...
  /* SHA appends to the end of each message 17 bytes of additional data
     ...
     allow tail reading to use a faster at_memcpy and then maybe some
     vectorization of the bswap. */

  ulong const * batch_data = (ulong const *)_batch_data;

  ulong batch_tail_data[ AT_SHA512_BATCH_MAX ] __attribute__((aligned(64)));
  ulong batch_tail_rem [ AT_SHA512_BATCH_MAX ] __attribute__((aligned(64)));

  uchar scratch[ AT_SHA512_BATCH_MAX*2UL*AT_SHA512_PRIVATE_BUF_MAX ] __attribute__((aligned(128)));
  do {
    ...
    for( ulong batch_idx=0UL; batch_idx<batch_cnt; batch_idx++ ) {
      ...
      ulong tail_data_sz  = sz & (AT_SHA512_PRIVATE_BUF_MAX-1UL);
      ulong tail_data_off = at_ulong_align_dn( sz,                AT_SHA512_PRIVATE_BUF_MAX );
      ulong tail_sz       = at_ulong_align_up( tail_data_sz+17UL, AT_SHA512_PRIVATE_BUF_MAX );

      batch_tail_data[ batch_idx ] = tail_data;
      batch_tail_rem [ batch_idx ] = tail_sz >> AT_SHA512_PRIVATE_LG_BUF_MAX;
...
#     if 1 /* See notes in at_sha256_batch_avx.c for more details here */
...
#     endif
      ...
      *((ulong *)(tail_data+tail_sz-16UL )) = at_ulong_bswap( sz>>61 );
  ...
  wwv_t zero       = wwv_zero();
  wwv_t one        = wwv_one();
  wwv_t wwv_128    = wwv_bcast( AT_SHA512_PRIVATE_BUF_MAX );
  ...
  wwv_t block_rem  = wwv_if( (1<<batch_cnt)-1,
                             wwv_add( wwv_shr( wwv_ld( batch_sz ), AT_SHA512_PRIVATE_LG_BUF_MAX ), tail_rem ), zero );
  for(;;) {
    int active_lane = wwv_ne( block_rem, zero );
    if( AT_UNLIKELY( !active_lane ) ) break;
