/* SHA3 AVX-512 Batch Implementation

   This file implements parallel SHA3-256 and SHA3-512 hashing using AVX-512.
   Up to 8 independent messages are processed simultaneously.

   The approach uses "lane-interleaved" processing where we maintain
   8 independent Keccak states and process them in parallel using
   512-bit vectors. */
...
#if AT_SHA3_512_BATCH_IMPL==2 || AT_SHA3_256_BATCH_IMPL==2
...
keccak_f1600_x8( ulong state[8][25] ) {
  ...
  /* Load all 25 lanes from 8 states into wwl_t vectors
     Each wwl_t holds lane[i] from all 8 states */
  wwl_t s[25];
  for( int i = 0; i < 25; i++ ) {
...
#endif /* AT_SHA3_512_BATCH_IMPL==2 || AT_SHA3_256_BATCH_IMPL==2 */

/**********************************************************************/
/* SHA3-512 Batch AVX-512                                              */
/**********************************************************************/

#if AT_SHA3_512_BATCH_IMPL==2

void *
at_sha3_512_batch_fini_avx512( at_sha3_512_batch_t * batch ) {
  ulong cnt = batch->cnt;
  ...
  /* If only 1-3 messages, or any message is too short, use sequential */
  ...
  /* Process each message (pad with duplicates if needed) */
  ...
  /* Finalize each state individually */
  for( ulong i = 0; i < cnt; i++ ) {
...
#endif /* AT_SHA3_512_BATCH_IMPL==2 */

/**********************************************************************/
/* SHA3-256 Batch AVX-512                                              */
/**********************************************************************/

#if AT_SHA3_256_BATCH_IMPL==2

void *
at_sha3_256_batch_fini_avx512( at_sha3_256_batch_t * batch ) {
  ulong cnt = batch->cnt;
...
#endif /* AT_SHA3_256_BATCH_IMPL==2 */
