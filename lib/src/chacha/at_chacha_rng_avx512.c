#include "at/crypto/at_chacha_rng.h"
...
static void
at_chacha_rng_refill_avx512( at_chacha_rng_t * rng,
                             ulong             rnd2_cnt ) {

  /* This function should only be called if the buffer is empty. */
  if( AT_UNLIKELY( rng->buf_off != rng->buf_fill ) ) {
    AT_LOG_CRIT(( "refill out of sync: buf_off=%lu buf_fill=%lu", rng->buf_off, rng->buf_fill ));
  }
  ...
  ulong idx = rng->buf_fill / AT_CHACHA_BLOCK_SZ;  /* really a right shift */
...
# undef QUARTER_ROUND
  ...
  rng->buf_fill += 16*AT_CHACHA_BLOCK_SZ;
}

void
at_chacha8_rng_refill_avx512( at_chacha_rng_t * rng ) {
  at_chacha_rng_refill_avx512( rng, 4UL );
}

void
at_chacha20_rng_refill_avx512( at_chacha_rng_t * rng ) {
  at_chacha_rng_refill_avx512( rng, 10UL );
}
