...
at_chacha_rng_refill_avx( at_chacha_rng_t * rng,
                          ulong             rnd2_cnt ) {
...
# undef QUARTER_ROUND
  ...
  ulong  slot = rng->buf_fill % (8*AT_CHACHA_BLOCK_SZ);
  uint * out  = (uint *)rng->buf + (slot*2*AT_CHACHA_BLOCK_SZ);
  ...
  rng->buf_fill += 8*AT_CHACHA_BLOCK_SZ;
}

void
at_chacha8_rng_refill_avx( at_chacha_rng_t * rng ) {
  at_chacha_rng_refill_avx( rng, 4UL );
}

void
at_chacha20_rng_refill_avx( at_chacha_rng_t * rng ) {
  at_chacha_rng_refill_avx( rng, 10UL );
}
