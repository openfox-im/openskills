...
/* This is not a proper header and so should not be included from
   anywhere besides at_base58.c and test_base58_avx.c.  As such, it has
   no include guard. */
...
count_leading_zeros_26( wuc_t in ) {
  ...
  ulong mask  = at_ulong_mask_lsb( 27 ) ^ (mask0 & at_ulong_mask_lsb( 26 )); /* Flips the low 26 bits and puts a 1 in bit 26 */
  return (ulong)at_ulong_find_lsb( mask );
}

static inline ulong
count_leading_zeros_32( wuc_t in ) {
  ulong mask = at_ulong_mask_lsb( 33 ) ^ (ulong)(uint)_mm256_movemask_epi8( _mm256_cmpeq_epi8( in, _mm256_setzero_si256() ));
  return (ulong)at_ulong_find_lsb( mask );
}
...
count_leading_zeros_45( wuc_t in0,
  ...
  ulong mask = at_ulong_mask_lsb( 46 ) ^ (((mask1 & at_ulong_mask_lsb( 13 ))<<32) | mask0);
  return (ulong)at_ulong_find_lsb( mask );
}
...
count_leading_zeros_64( wuc_t in0,
  ...
  return (ulong)at_ulong_find_lsb_w_default( mask, 64 );
}
