/* Avatar Base58 implementation */

#include "at_base58.h"
#include "at/infra/at_util.h"

/* Disable AVX for now until SIMD headers are ported */
#undef AT_HAS_AVX
#define AT_HAS_AVX 0
...
#include "at_base58_tmpl.c"

/* General-purpose Base58 encoding/decoding for arbitrary lengths.
   Uses standard "divide by 58" algorithm - O(n^2) but works for any size. */

static char const base58_alphabet[] = "123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz";

char *
at_base58_encode( uchar const * bytes,
                  ulong         bytes_sz,
                  ulong       * opt_len,
                  char        * out ) {
  ...
  if( bytes_sz == 0 ) {
    out[0] = '\0';
    if( opt_len ) *opt_len = 0;
    return out;
  }

  /* Count leading zeros */
  ulong leading_zeros = 0;
  while( leading_zeros < bytes_sz && bytes[leading_zeros] == 0 ) {
    leading_zeros++;
  }

  /* Allocate temporary buffer for base conversion.
     Max output size is ceil(bytes_sz * log(256) / log(58)) ≈ bytes_sz * 1.37 + 1 */
  ulong max_out = (bytes_sz * 138 / 100) + 2;
  uchar tmp[512];  /* Stack buffer for reasonable sizes */
  uchar * buf = tmp;
  if( max_out > sizeof(tmp) ) {
    /* For very large inputs, this would need heap allocation.
  ...
  /* Copy input to working buffer (we'll modify it) */
  uchar work[256];
  if( bytes_sz > sizeof(work) ) {
    out[0] = '\0';
    if( opt_len ) *opt_len = 0;
    return out;
  }
  at_memcpy( work, bytes, bytes_sz );

  /* Convert to base58 using repeated division */
  ulong out_idx = 0;
  ulong start = leading_zeros;  /* Skip leading zeros in division */

  while( start < bytes_sz ) {
    uint carry = 0;
    ulong new_start = bytes_sz;  /* Track where non-zero values start */

    for( ulong i = start; i < bytes_sz; i++ ) {
      uint val = carry * 256 + work[i];
      work[i] = (uchar)(val / 58);
      carry = val % 58;

      if( work[i] != 0 && new_start == bytes_sz ) {
        new_start = i;
      }
    }

    buf[out_idx++] = (uchar)carry;
    start = new_start;
  }

  /* Build output: leading '1's for each leading zero byte */
  ulong pos = 0;
  for( ulong i = 0; i < leading_zeros; i++ ) {
    out[pos++] = '1';
  }

  /* Append base58 digits in reverse order */
  while( out_idx > 0 ) {
    out[pos++] = base58_alphabet[buf[--out_idx]];
  }

  out[pos] = '\0';
  if( opt_len ) *opt_len = pos;
  return out;
}

uchar *
at_base58_decode( char const * encoded,
                  uchar      * out,
                  ulong        out_sz ) {
  ...
  /* Handle empty input/output case */
  if( out_sz == 0 ) {
    /* Empty output is only valid if encoded is also empty */
    return ( encoded[0] == '\0' ) ? out : NULL;
  }

  /* Count string length and leading '1's */
  ulong str_len = 0;
  ulong leading_ones = 0;

  while( encoded[str_len] ) {
    if( encoded[str_len] == '1' && str_len == leading_ones ) {
      leading_ones++;
    }
    str_len++;
  }
  ...
  if( str_len == 0 ) {
    at_memset( out, 0, out_sz );
    return out;
  }

  /* Decode base58 to binary using multiplication */
  uchar work[256];
  if( out_sz > sizeof(work) ) {
    return NULL;
  }
  at_memset( work, 0, out_sz );
  ...
  for( ulong i = 0; i < str_len; i++ ) {
    ...
    /* Multiply work by 58 and add val */
    uint carry = (uint)val;
    for( ulong j = out_sz; j > 0; j-- ) {
      uint v = (uint)work[j-1] * 58 + carry;
      work[j-1] = (uchar)(v & 0xFF);
      carry = v >> 8;
    }

    if( carry != 0 ) {
      return NULL;  /* Overflow - result doesn't fit in out_sz bytes */
    }
  }

  /* Verify leading zeros match leading '1's */
  ulong leading_zeros = 0;
  while( leading_zeros < out_sz && work[leading_zeros] == 0 ) {
    leading_zeros++;
  }

  if( leading_zeros != leading_ones ) {
    return NULL;  /* Mismatch in leading zeros */
  }

  at_memcpy( out, work, out_sz );
  return out;
}
