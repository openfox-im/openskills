/* Declares conversion functions to/from base58 for a specific size of
   ...
   Expects that enc_table_N, dec_table_N, and AT_BASE58_ENCODED_N_SZ
   exist (substituting the numeric value of N).

   This file is safe for inclusion multiple times. */

#define BYTE_CNT     ((ulong) N)
#define SUFFIX(s)    AT_EXPAND_THEN_CONCAT3(s,_,N)
#define ENCODED_SZ() AT_EXPAND_THEN_CONCAT3(AT_BASE58_ENCODED_, N, _SZ)
#define RAW58_SZ     (INTERMEDIATE_SZ*5UL)

#if AT_HAS_AVX
#define INTERMEDIATE_SZ_W_PADDING AT_ULONG_ALIGN_UP( INTERMEDIATE_SZ, 4UL )
#else
#define INTERMEDIATE_SZ_W_PADDING INTERMEDIATE_SZ
#endif

char *
SUFFIX(at_base58_encode)( uchar const * bytes,
                          ulong       * opt_len,
                          char        * out    ){
...
# endif
  ...
  for( ulong i=0UL; i<BINARY_SZ; i++ ) binary[ i ] = at_uint_bswap( at_uint_load_4( &bytes[ i*sizeof(uint) ] ) );
  ...
  at_memset( intermediate, 0, INTERMEDIATE_SZ_W_PADDING * sizeof(ulong) );
...
# elif N==64
  ...
  ulong raw_leading_0s = at_ulong_if( raw_leading_0s_part1<64UL, raw_leading_0s_part1, 64UL+raw_leading_0s_part2 );
...
# endif
  ...
  out[ RAW58_SZ-skip ] = '\0';
  at_ulong_store_if( !!opt_len, opt_len, RAW58_SZ-skip );
  return out;
}

uchar *
SUFFIX(at_base58_decode)( char const * encoded,
                          uchar      * out      ) {
  ...
  for( ; char_cnt<ENCODED_SZ(); char_cnt++ ) {
    ...
    idx = at_ulong_min( idx, BASE58_INVERSE_TABLE_SENTINEL );
    if( AT_UNLIKELY( base58_inverse[ idx ] == BASE58_INVALID_CHAR ) ) return NULL;
  }

  if( AT_UNLIKELY( char_cnt == ENCODED_SZ() ) ) return NULL; /* too long */
  ...
  if( AT_UNLIKELY( binary[ 0UL ] > 0xFFFFFFFFUL ) ) return NULL;

  /* Convert each term to big endian for the final output */

  uint * out_as_uint = (uint*)out;
  for( ulong i=0UL; i<BINARY_SZ; i++ ) {
    AT_STORE( uint, &out_as_uint[ i ], at_uint_bswap( (uint)binary[ i ] ) );
  }
  ...
  for( ; leading_zero_cnt<BYTE_CNT; leading_zero_cnt++ ) {
    ...
    if( AT_UNLIKELY( encoded[ leading_zero_cnt ] != '1' ) ) return NULL;
  }
  if( AT_UNLIKELY( encoded[ leading_zero_cnt ] == '1' ) ) return NULL;
  return out;
}
