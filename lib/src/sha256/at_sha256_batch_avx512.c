...
#include "at/crypto/at_sha256_constants.h"
#include "at/infra/simd/at_avx512.h"
#include "at/infra/simd/at_avx.h"

AT_STATIC_ASSERT( AT_SHA256_BATCH_MAX==16UL, compat );

void
at_sha256_private_batch_avx( ulong          batch_cnt,
                             void const *   batch_data,
                             ulong const *  batch_sz,
                             void * const * batch_hash );

void
at_sha256_private_batch_avx512( ulong          batch_cnt,
...
# endif
  ...
  if( AT_UNLIKELY( batch_cnt<MIN_BATCH_CNT ) ) {
    at_sha256_private_batch_avx( batch_cnt, _batch_data, batch_sz, _batch_hash );
    return;
  }
...
# undef MIN_BATCH_CNT
  ...
  ulong batch_tail_data[ AT_SHA256_BATCH_MAX ] __attribute__((aligned(64)));
  ulong batch_tail_rem [ AT_SHA256_BATCH_MAX ] __attribute__((aligned(64)));

  uchar scratch[ AT_SHA256_BATCH_MAX*2UL*AT_SHA256_PRIVATE_BUF_MAX ] __attribute__((aligned(128)));
  do {
    ...
    for( ulong batch_idx=0UL; batch_idx<batch_cnt; batch_idx++ ) {
      ...
      ulong tail_data_sz  = sz & (AT_SHA256_PRIVATE_BUF_MAX-1UL);
      ulong tail_data_off = at_ulong_align_dn( sz,               AT_SHA256_PRIVATE_BUF_MAX );
      ulong tail_sz       = at_ulong_align_up( tail_data_sz+9UL, AT_SHA256_PRIVATE_BUF_MAX );

      batch_tail_data[ batch_idx ] = tail_data;
      batch_tail_rem [ batch_idx ] = tail_sz >> AT_SHA256_PRIVATE_LG_BUF_MAX;
...
#     if 1
      /* Quick experiments found that, once again, straight memcpy is
         much slower than a at_memcpy is slightly slower than a
         site-optimized handrolled memcpy (at_memcpy would be less L1I
      ...
      while( rem>= 8UL ) { *(ulong  *)dst = AT_LOAD( ulong,  src );             dst +=  8UL; src +=  8UL; rem -=  8UL; }
      if   ( rem>= 4UL ) { *(uint   *)dst = AT_LOAD( uint,   src );             dst +=  4UL; src +=  4UL; rem -=  4UL; }
      if   ( rem>= 2UL ) { *(ushort *)dst = AT_LOAD( ushort, src );             dst +=  2UL; src +=  2UL; rem -=  2UL; }
      if   ( rem       ) { *(uchar  *)dst = AT_LOAD( uchar,  src );             dst++;                                 }
      *(uchar *)dst = (uchar)0x80;
#     else
      at_memcpy( (void *)tail_data, (void const *)(data + tail_data_off), tail_data_sz );
      *((uchar *)(tail_data+tail_data_sz)) = (uchar)0x80;
#     endif

      *((ulong *)(tail_data+tail_sz-8UL )) = at_ulong_bswap( sz<<3 );
    }
  } while(0);

  wwu_t s0 = wwu_bcast( AT_SHA256_INITIAL_A );
  wwu_t s1 = wwu_bcast( AT_SHA256_INITIAL_B );
  wwu_t s2 = wwu_bcast( AT_SHA256_INITIAL_C );
  wwu_t s3 = wwu_bcast( AT_SHA256_INITIAL_D );
  wwu_t s4 = wwu_bcast( AT_SHA256_INITIAL_E );
  wwu_t s5 = wwu_bcast( AT_SHA256_INITIAL_F );
  wwu_t s6 = wwu_bcast( AT_SHA256_INITIAL_G );
  wwu_t s7 = wwu_bcast( AT_SHA256_INITIAL_H );

  wwv_t zero       = wwv_zero();
  wwv_t one        = wwv_one();
  wwv_t wwv_64     = wwv_bcast( AT_SHA256_PRIVATE_BUF_MAX );
  ...
  wwv_t block_rem_lo = wwv_if( ((1<<batch_cnt)-1) & 0xff,
                               wwv_add( wwv_shr( wwv_ld( batch_sz   ), AT_SHA256_PRIVATE_LG_BUF_MAX ), tail_rem_lo ), zero );
  wwv_t block_rem_hi = wwv_if( ((1<<batch_cnt)-1) >> 8,
                               wwv_add( wwv_shr( wwv_ld( batch_sz+8 ), AT_SHA256_PRIVATE_LG_BUF_MAX ), tail_rem_hi ), zero );

  for(;;) {
    int active_lane_lo = wwv_ne( block_rem_lo, zero );
    int active_lane_hi = wwv_ne( block_rem_hi, zero );
    if( AT_UNLIKELY( !(active_lane_lo | active_lane_hi) ) ) break;
...
#   define SHA_CORE(xi,ki)                                                           \
    ...
    SHA_CORE( x0, wwu_bcast( at_sha256_K[ 0] ) );
    SHA_CORE( x1, wwu_bcast( at_sha256_K[ 1] ) );
    SHA_CORE( x2, wwu_bcast( at_sha256_K[ 2] ) );
    SHA_CORE( x3, wwu_bcast( at_sha256_K[ 3] ) );
    SHA_CORE( x4, wwu_bcast( at_sha256_K[ 4] ) );
    SHA_CORE( x5, wwu_bcast( at_sha256_K[ 5] ) );
    SHA_CORE( x6, wwu_bcast( at_sha256_K[ 6] ) );
    SHA_CORE( x7, wwu_bcast( at_sha256_K[ 7] ) );
    SHA_CORE( x8, wwu_bcast( at_sha256_K[ 8] ) );
    SHA_CORE( x9, wwu_bcast( at_sha256_K[ 9] ) );
    SHA_CORE( xa, wwu_bcast( at_sha256_K[10] ) );
    SHA_CORE( xb, wwu_bcast( at_sha256_K[11] ) );
    SHA_CORE( xc, wwu_bcast( at_sha256_K[12] ) );
    SHA_CORE( xd, wwu_bcast( at_sha256_K[13] ) );
    SHA_CORE( xe, wwu_bcast( at_sha256_K[14] ) );
    SHA_CORE( xf, wwu_bcast( at_sha256_K[15] ) );
    for( ulong i=16UL; i<64UL; i+=16UL ) {
      x0 = wwu_add( wwu_add( x0, sigma0(x1) ), wwu_add( sigma1(xe), x9 ) ); SHA_CORE( x0, wwu_bcast( at_sha256_K[i     ] ) );
      x1 = wwu_add( wwu_add( x1, sigma0(x2) ), wwu_add( sigma1(xf), xa ) ); SHA_CORE( x1, wwu_bcast( at_sha256_K[i+ 1UL] ) );
      x2 = wwu_add( wwu_add( x2, sigma0(x3) ), wwu_add( sigma1(x0), xb ) ); SHA_CORE( x2, wwu_bcast( at_sha256_K[i+ 2UL] ) );
      x3 = wwu_add( wwu_add( x3, sigma0(x4) ), wwu_add( sigma1(x1), xc ) ); SHA_CORE( x3, wwu_bcast( at_sha256_K[i+ 3UL] ) );
      x4 = wwu_add( wwu_add( x4, sigma0(x5) ), wwu_add( sigma1(x2), xd ) ); SHA_CORE( x4, wwu_bcast( at_sha256_K[i+ 4UL] ) );
      x5 = wwu_add( wwu_add( x5, sigma0(x6) ), wwu_add( sigma1(x3), xe ) ); SHA_CORE( x5, wwu_bcast( at_sha256_K[i+ 5UL] ) );
      x6 = wwu_add( wwu_add( x6, sigma0(x7) ), wwu_add( sigma1(x4), xf ) ); SHA_CORE( x6, wwu_bcast( at_sha256_K[i+ 6UL] ) );
      x7 = wwu_add( wwu_add( x7, sigma0(x8) ), wwu_add( sigma1(x5), x0 ) ); SHA_CORE( x7, wwu_bcast( at_sha256_K[i+ 7UL] ) );
      x8 = wwu_add( wwu_add( x8, sigma0(x9) ), wwu_add( sigma1(x6), x1 ) ); SHA_CORE( x8, wwu_bcast( at_sha256_K[i+ 8UL] ) );
      x9 = wwu_add( wwu_add( x9, sigma0(xa) ), wwu_add( sigma1(x7), x2 ) ); SHA_CORE( x9, wwu_bcast( at_sha256_K[i+ 9UL] ) );
      xa = wwu_add( wwu_add( xa, sigma0(xb) ), wwu_add( sigma1(x8), x3 ) ); SHA_CORE( xa, wwu_bcast( at_sha256_K[i+10UL] ) );
      xb = wwu_add( wwu_add( xb, sigma0(xc) ), wwu_add( sigma1(x9), x4 ) ); SHA_CORE( xb, wwu_bcast( at_sha256_K[i+11UL] ) );
      xc = wwu_add( wwu_add( xc, sigma0(xd) ), wwu_add( sigma1(xa), x5 ) ); SHA_CORE( xc, wwu_bcast( at_sha256_K[i+12UL] ) );
      xd = wwu_add( wwu_add( xd, sigma0(xe) ), wwu_add( sigma1(xb), x6 ) ); SHA_CORE( xd, wwu_bcast( at_sha256_K[i+13UL] ) );
      xe = wwu_add( wwu_add( xe, sigma0(xf) ), wwu_add( sigma1(xc), x7 ) ); SHA_CORE( xe, wwu_bcast( at_sha256_K[i+14UL] ) );
      xf = wwu_add( wwu_add( xf, sigma0(x0) ), wwu_add( sigma1(xd), x8 ) ); SHA_CORE( xf, wwu_bcast( at_sha256_K[i+15UL] ) );
    }
